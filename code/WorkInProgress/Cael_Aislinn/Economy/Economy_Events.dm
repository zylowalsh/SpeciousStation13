	...
	if(affected_dest.viable_random_events.len)
		...
		switch(event_type)
			...
			if(INDUSTRIAL_ACCIDENT)
				dearer_goods = list(ECON_EMERGENCY, BIOMEDICAL, ROBOTICS)
			if(BIOHAZARD_OUTBREAK)
				dearer_goods = list(BIOMEDICAL, ECON_GAS)
			if(PIRATES)
			...
			if(ALIEN_RAIDERS)
				...
				cheaper_goods = list(ECON_GAS, MINERALS)
			if(AI_LIBERATION)
				dearer_goods = list(ECON_EMERGENCY, ECON_GAS, MAINTENANCE)
			if(MOURNING)
