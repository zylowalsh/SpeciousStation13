...
"}
				...
				if(4.0)
					...
					if ((istype(src.active1, /datum/record) && dataCore.general.Find(src.active1)))
					...
					if ((istype(src.active2, /datum/record) && dataCore.medical.Find(src.active2)))
...
/obj/machinery/computer/med_data/Topic(href, href_list)
	...
	if (!( dataCore.general.Find(src.active1) ))
		src.active1 = null

	if (!( dataCore.medical.Find(src.active2) ))
		src.active2 = null
	...
	if ((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if (src.authenticated)
			...
			if(href_list["vir"])
				var/datum/record/v = locate(href_list["vir"])
			...
			if (href_list["field"])
				...
				switch(href_list["field"])
					if("fingerprint")
						if (istype(src.active1, /datum/record))
					...
					if("mi_dis")
						if (istype(src.active2, /datum/record))
					...
					if("vir_name")
						var/datum/record/v = locate(href_list["edit_vir"])
						if (v)
			...
			if (href_list["d_rec"])
				...
				var/datum/record/M = locate(href_list["d_rec"])
			...
			if (href_list["new"])
				if ((istype(src.active1, /datum/record) && !( istype(src.active2, /datum/record) )))
			...
			if (href_list["del_c"])
				if ((istype(src.active2, /datum/record) && src.active2.fields[text("com_[]", href_list["del_c"])]))
