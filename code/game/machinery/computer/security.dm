		...
		if (authenticated)
			switch(screen)
				...
				if(3.0)
					...
					if ((istype(active2, /datum/record) && dataCore.security.Find(active2)))
...
//RECORD DELETE
			...
			if ("Add Entry")
				if (!( istype(active2, /datum/record) ))
					return
				var/a2 = active2
			...
			if ("Delete Entry")
				if ((istype(active2, /datum/record) && active2.fields[text("com_[]", href_list["del_c"])]))
					active2.fields[text("com_[]", href_list["del_c"])] = "<B>Deleted</B>"
//RECORD CREATE
			if ("New Record (Security)")
				if ((istype(active1, /datum/record) && !( istype(active2, /datum/record) )))
